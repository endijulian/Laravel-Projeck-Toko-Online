...
class TransaksiController extends Controller
{
    public function addCart(Request $request)
    {
        ...
        //mencari jumlah poduk, atas jumlah produk itu sendiri didalam tabel keranjang
        $jumlah_barang_cart = Keranjang::where('kd_produk', $data['kd_produk'])->sum('jumlah');

        $stok_hasil = $stok_produk - $jumlah_barang_cart;

        //jika stok hasil lebih kecil dari jumlah yang di inputkan maka akan menampilkan tulisan
    ...
    {
        $query = DB::select('SELECT MAX(RIGHT(no_faktur,6)) AS max_faktur FROM transaksi WHERE DATE(tanggal_penjualan)= CURDATE()');
